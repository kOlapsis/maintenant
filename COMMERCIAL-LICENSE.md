# Maintenant Commercial License

**Version 1.0 -- March 2026**

This Commercial License Agreement ("Agreement") is between Benjamin Touchard,
operating as Kolapsis ("Licensor"), and the individual or entity subscribing to
a Maintenant Pro plan ("Licensee").

## 1. Dual Licensing

Maintenant is available under two licenses:

- **AGPL-3.0** -- free for everyone, with the obligation to share modifications
  under the same terms (see `LICENSE`).
- **Commercial License** -- available to Pro subscribers, granting the rights
  described below.

If you do not hold an active Pro subscription, the AGPL-3.0 applies.

## 2. Grant of Rights

Subject to the terms of this Agreement, the Licensor grants the Licensee a
**non-exclusive, non-transferable, worldwide** license to:

1. Use Maintenant, including Pro features, in production.
2. Modify the source code for internal use **without** the AGPL-3.0 obligation
   to publish those modifications.
3. Receive email support at license@maintenant.dev.

## 3. Scope

- Each Pro subscription covers **one deployment instance** of Maintenant.
- Additional instances require additional subscriptions.
- The license does **not** grant the right to redistribute, sublicense, or
  resell the software.

## 4. Subscription & Pricing

| Plan          | Price         |
|---------------|---------------|
| Pro Monthly   | 29 EUR / month |
| Pro Annual    | 290 EUR / year |

Subscriptions are managed directly inside the app or at [maintenant.dev](https://maintenant.dev).

## 5. Term & Termination

- This license is valid **for the duration of the active subscription**.
- If the subscription lapses, the Licensee must either renew, remove Pro
  features, or comply with the AGPL-3.0 for any continued use.
- The Licensor may terminate this Agreement immediately if the Licensee
  breaches its terms.

## 6. Warranty Disclaimer

THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND, EXPRESS OR
IMPLIED. THE LICENSOR SHALL NOT BE LIABLE FOR ANY CLAIM, DAMAGES, OR OTHER
LIABILITY ARISING FROM THE USE OF THE SOFTWARE.

## 7. Contact

For licensing questions, volume pricing, or custom agreements:

**license@maintenant.dev**
