...
// external_id and be stored (the cascade that was broken before the fix).
func TestHandleAgentEvent_PersistsSnapshotForAgentContainer(t *testing.T) {
	extID := "abc123def4567890"
	c := &container.Container{ID: 42, ExternalID: extID, Name: "demo"}
	...
	assert.Equal(t, int64(42), snap.ContainerID)
	assert.Equal(t, 12.5, snap.CPUPercent)
	require.NotNil(t, snap.AgentID)
