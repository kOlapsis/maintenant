...
func applyMigration21Up(t *testing.T, rawDB *sql.DB) {
	t.Helper()
	sqlBytes, err := os.ReadFile("migrations/sqlite/21_containers_cascade_delete.up.sql")
...
func applyMigration21Down(t *testing.T, rawDB *sql.DB) {
	t.Helper()
	sqlBytes, err := os.ReadFile("migrations/sqlite/21_containers_cascade_delete.down.sql")
