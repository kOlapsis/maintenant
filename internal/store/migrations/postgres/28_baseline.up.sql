-- maintenant — PostgreSQL baseline schema.
-- Derived from uuid_schema.sql at SQLite migration head 28: installs the whole
-- schema in one application on an empty database. Numbered 28 so the schema
-- version stays comparable across engines; every migration from 29 onward is
-- written for both engines under the same number, and a dedicated test fails
-- on any divergence between the two heads.
-- Type mapping: DOUBLE PRECISION -> DOUBLE PRECISION, BYTEA -> BYTEA; everything else is
-- already in the portable subset (TEXT keys, BIGINT epoch seconds, INTEGER
-- 0/1 booleans). schema_meta is not carried over: it marks the local file's
-- historical conversions and has no meaning here.
CREATE TABLE agents (
    id                TEXT PRIMARY KEY NOT NULL,            -- UUID (agent-generated) or LocalAgent sentinel
    public_key        BYTEA,                                 -- raw 32 bytes Ed25519; NULL for the local sentinel
...
CREATE TABLE resource_alert_configs (
    ...
    mem_threshold DOUBLE PRECISION NOT NULL DEFAULT 90.0,
