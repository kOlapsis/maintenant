-- Issue #25: retention cleanup fails with "FOREIGN KEY constraint failed" when
-- deleting archived containers. The three child tables of `containers` had no
-- ON DELETE CASCADE, so any surviving transition / snapshot / alert config
-- blocked the parent purge. Rebuild them with CASCADE so retention can drop an
-- archived container and its history atomically.

-- 1. Rebuild state_transitions with ON DELETE CASCADE.
...
INSERT INTO state_transitions_new (id, container_id, previous_state, new_state,
...
-- 2. Rebuild resource_snapshots with ON DELETE CASCADE.
...
INSERT INTO resource_snapshots_new (id, container_id, cpu_percent, mem_used, mem_limit,
...
-- 3. Rebuild resource_alert_configs with ON DELETE CASCADE.
CREATE TABLE resource_alert_configs_new (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    container_id INTEGER NOT NULL UNIQUE REFERENCES containers(id) ON DELETE CASCADE,
...
INSERT INTO resource_alert_configs_new (id, container_id, cpu_threshold, mem_threshold,
...
ALTER TABLE resource_alert_configs_new RENAME TO resource_alert_configs;
