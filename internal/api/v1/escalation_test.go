...
func (m *escalationTestStore) SelectOrphanPendingDeliveries(_ context.Context, _ time.Time) ([]*escalation.Delivery, error) {
	return nil, nil
}
...
func escalationMux(t *testing.T) *http.ServeMux {
	...
	mux.HandleFunc("POST /api/v1/escalation-policies", requireCapability(extension.CapAlertEscalation, h.HandleCreatePolicy))
	mux.HandleFunc("GET /api/v1/escalation-policies", requireCapability(extension.CapAlertEscalation, h.HandleListPolicies))
	mux.HandleFunc("POST /api/v1/escalation-policies/overlap-probe", requireCapability(extension.CapAlertEscalation, h.HandleOverlapProbe))
	mux.HandleFunc("GET /api/v1/escalation-policies/{id}", requireCapability(extension.CapAlertEscalation, h.HandleGetPolicy))
	mux.HandleFunc("DELETE /api/v1/escalation-policies/{id}", requireCapability(extension.CapAlertEscalation, h.HandleDeletePolicy))
	mux.HandleFunc("PUT /api/v1/escalation-policies/{id}", requireCapability(extension.CapAlertEscalation, h.HandleUpdatePolicy))
	mux.HandleFunc("PATCH /api/v1/escalation-policies/{id}/active", requireCapability(extension.CapAlertEscalation, h.HandleSetPolicyActive))
	mux.HandleFunc("GET /api/v1/escalation-policies/{id}/runs", requireCapability(extension.CapAlertEscalation, h.HandleListPolicyRuns))
	mux.HandleFunc("GET /api/v1/escalation-runs/{run_id}", requireCapability(extension.CapAlertEscalation, h.HandleGetRun))
	mux.HandleFunc("GET /api/v1/alerts/{alert_id}/escalation-runs", requireCapability(extension.CapAlertEscalation, h.HandleListAlertRuns))
	return mux
}
