...
function stateColor(state: string): string {
  switch (state) {
    ...
    case 'starting':
    case 'assigned':
      return 'text-mnt-secondary'
    default:
      return 'text-mnt-status-warn'
  }
}
...
function stateDot(state: string): string {
  switch (state) {
    ...
    case 'assigned':
      return 'bg-mnt-sev-neutral-solid'
    default:
      return 'bg-mnt-sev-warning-solid'
  }
}
...
<template>
  <div class="overflow-y-auto p-3 sm:p-6">
    <div class="max-w-7xl mx-auto">
      <!-- Page header -->
      <div class="mb-6">
        <h1 class="text-2xl font-black text-mnt-primary">Tasks</h1>
        <p class="mt-1 text-sm text-mnt-muted">All Swarm tasks</p>
      </div>
      ...
      <div v-if="loading" class="flex items-center justify-center py-16">
        <span class="text-sm text-mnt-muted">Loading tasks…</span>
      </div>
      ...
      >
        <ClipboardList :size="32" class="mx-auto mb-3 text-mnt-muted" />
      ...
      <!-- Task list -->
      <div v-else class="bg-mnt-surface rounded-xl border border-mnt-default overflow-hidden">
        <!-- Table header -->
        <div
          class="grid grid-cols-[1fr_1fr_80px_1fr_120px_80px] gap-3 px-4 py-2 border-b border-mnt-default"
        >
          ...
          <span class="text-[10px] text-mnt-muted font-bold uppercase tracking-widest"
            >Service</span
          >
          <span class="text-[10px] text-mnt-muted font-bold uppercase tracking-widest">Slot</span>
          <span class="text-[10px] text-mnt-muted font-bold uppercase tracking-widest">Node</span>
          <span class="text-[10px] text-mnt-muted font-bold uppercase tracking-widest">State</span>
          <span class="text-[10px] text-mnt-muted font-bold uppercase tracking-widest text-right"
            >When</span
          >
        </div>
        ...
        <div class="divide-y divide-slate-800/60">
          <div
            ...
            :key="task.task_id"
            class="grid grid-cols-[1fr_1fr_80px_1fr_120px_80px] gap-3 px-4 py-2.5 hover:bg-mnt-elevated transition-all items-center"
          >
            <!-- Task ID -->
            ...
            <span class="flex items-center gap-1.5 min-w-0">
              <span class="text-xs text-mnt-secondary font-medium truncate">{{
                task.service_name
              }}</span>
              <HostBadge
            ...
            <!-- Slot -->
            <span class="text-xs text-mnt-muted font-mono">#{{ task.slot }}</span>

            <!-- Node -->
            <span class="text-xs text-mnt-muted truncate">{{ task.node_hostname || '—' }}</span>
            ...
            <div class="flex items-center gap-2 min-w-0">
              ...
              <span v-if="task.error" class="text-[10px] text-mnt-status-down truncate" :title="task.error">
                {{ task.error }}
              </span>
              <span
                v-else-if="task.exit_code !== null && task.exit_code !== 0"
                class="text-[10px] text-mnt-status-down flex-shrink-0"
              >
                exit {{ task.exit_code }}
              </span>
            </div>
            ...
            <!-- When -->
            <span class="text-xs text-mnt-muted tabular-nums text-right">{{
