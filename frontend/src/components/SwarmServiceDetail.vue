...
function cpuColor(cpu: number | null): string {
  if (cpu === null) return 'bg-pb-elevated'
  if (cpu > 80) return 'bg-pb-sev-incident-solid'
  if (cpu > 50) return 'bg-pb-sev-warning-solid'
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div v-if="loading" class="flex items-center justify-center py-16">
      <span class="text-sm text-pb-muted">Loading service…</span>
    </div>
    ...
    <template v-else-if="detail">
      ...
      <div class="px-5 pt-4 pb-3 border-b border-pb-default">
        ...
        <!-- Image -->
        <div class="mt-3">
          <p class="text-[10px] text-pb-muted font-bold uppercase tracking-widest mb-1">Image</p>
          <div class="flex items-center gap-2">
            <span class="text-xs text-pb-secondary font-mono truncate">{{ imageTag(detail.image).name }}</span>
            <span class="text-[10px] font-bold text-pb-muted bg-pb-elevated px-1.5 py-0.5 rounded font-mono">
        ...
        <div class="mt-3 flex gap-6 text-xs text-pb-muted">
          <span>Created <span class="text-pb-muted">{{ timeAgo(detail.created_at) }}</span></span>
          <span v-if="detail.update_status?.state">
            Update <span :class="detail.update_status.state === 'completed' ? 'text-pb-status-ok' : 'text-pb-status-warn'">{{ detail.update_status.state }}</span>
          </span>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto px-5 py-4">
        ...
        <template v-else-if="activeTab === 'ports'">
          <div v-if="detail.ports.length === 0" class="text-sm text-pb-muted py-4 text-center">
            No published ports
          </div>
          <div v-else class="space-y-1">
            <div
              ...
              :key="i"
              class="bg-pb-primary rounded-lg border border-pb-default px-4 py-3 flex items-center justify-between"
            >
              <div class="flex items-center gap-3">
                <span class="text-sm font-mono text-pb-primary">
                  {{ port.published_port ? `:${port.published_port}` : 'auto' }}
                  <span class="text-pb-muted mx-1">→</span>
                  :{{ port.target_port }}
                </span>
                <span class="text-[10px] font-bold uppercase tracking-wider text-pb-muted bg-pb-elevated border border-pb-default px-1.5 py-0.5 rounded">
                  {{ port.protocol }}
              ...
              <span class="text-[10px] font-bold uppercase tracking-wider text-pb-muted">
                {{ port.publish_mode }}
              </span>
            </div>
          </div>
        ...
        <template v-else-if="activeTab === 'networks'">
          <div v-if="detail.networks.length === 0" class="text-sm text-pb-muted py-4 text-center">
        ...
        <template v-else-if="activeTab === 'resources'">
          ...
          >
            ...
            <div v-else-if="taskResources.length === 0" class="text-sm text-pb-muted py-4 text-center">
              ...
              >
                <div class="flex items-center justify-between mb-2">
                  <div class="flex items-center gap-2">
                    ...
                    <span v-if="tr.node_hostname" class="text-xs text-pb-muted font-mono">{{ tr.node_hostname }}</span>
                  </div>
                  <span v-if="tr.timestamp" class="text-[10px] text-pb-muted">{{ timeAgo(tr.timestamp) }}</span>
                </div>

                <!-- CPU bar -->
                <div class="mb-2">
                  <div class="flex items-center justify-between mb-1">
                    <span class="text-[10px] text-pb-muted font-bold uppercase tracking-widest">CPU</span>
                ...
                <div class="mb-2">
                  <div class="flex items-center justify-between mb-1">
                    <span class="text-[10px] text-pb-muted font-bold uppercase tracking-widest">Memory</span>
                    <span class="text-xs text-pb-muted tabular-nums">
                      {{ formatBytes(tr.mem_used) }} / {{ formatBytes(tr.mem_limit) }}
                      <span v-if="tr.mem_percent !== null" class="text-pb-muted ml-1">({{ tr.mem_percent.toFixed(1) }}%)</span>
                    </span>
                  </div>
                  <div class="h-1.5 bg-pb-primary border border-pb-default rounded-full overflow-hidden">
                    <div
                ...
                <!-- Network -->
                <div v-if="tr.net_rx_bytes !== null && tr.net_rx_bytes >= 0" class="flex items-center gap-4 text-xs text-pb-muted">
    ...
    <div v-else class="flex items-center justify-center py-16">
      <span class="text-sm text-pb-muted">Service not found.</span>
    </div>
  </div>
</template>
