...
<template>
  <div
    ...
    :class="search.isValid.value ? 'border-pb-default' : 'border-red-500'"
  >
    <input
      ref="inputRef"
      type="text"
      ...
      class="w-32 bg-transparent text-xs text-pb-primary placeholder:text-pb-muted outline-none sm:w-48"
      @input="onInput"
      @keydown="onKeydown"
    />
    ...
    <span
      ...
      :class="search.matches.value.length > 0 ? 'text-pb-muted' : 'text-pb-muted'"
    >{{ matchDisplay }}</span>
    ...
    <button
      ...
      :class="search.isCaseSensitive.value
        ...
        : 'text-pb-muted hover:text-pb-secondary'"
      title="Match Case"
    ...
    <!-- Navigation -->
    <button
      class="shrink-0 rounded p-0.5 text-pb-muted transition-colors hover:text-pb-secondary"
