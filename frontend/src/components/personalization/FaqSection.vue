...
<template>
  <div class="space-y-4">
    <h3 class="text-[10px] text-pb-muted font-bold uppercase tracking-widest">FAQ</h3>

    <div class="space-y-3">
      <div
        v-for="(item, idx) in store.faqItems"
        :key="item.id"
        class="bg-pb-primary border border-pb-default rounded-xl p-4 space-y-2"
      >
        <div class="flex items-center gap-2">
          ...
          <input
            :value="item.question"
            maxlength="200"
            class="flex-1 bg-transparent text-pb-primary text-sm focus:outline-none font-medium"
        ...
        <textarea
          ...
          maxlength="4000"
          rows="3"
          class="w-full bg-pb-surface border border-pb-default rounded-lg px-3 py-2 text-pb-secondary text-sm font-mono focus:outline-none focus:border-pb-default resize-none"
    ...
    <p v-if="faqError" class="text-xs text-pb-status-down">{{ faqError }}</p>
  </div>
</template>
