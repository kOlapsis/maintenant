...
function containerStateStyle(state: string): string {
  ...
  if (s === 'running') return 'text-pb-status-ok bg-pb-status-ok border-emerald-400/20'
  if (s === 'waiting') return 'text-pb-status-warn bg-pb-status-warn border-pb-sev-warning'
  if (s === 'terminated') return 'text-pb-muted bg-pb-elevated border-pb-default'
  return 'text-pb-muted bg-pb-elevated border-pb-default'
}
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div v-if="loading" class="flex items-center justify-center py-16">
      <span class="text-sm text-pb-muted">Loading pod…</span>
    </div>
    ...
    <template v-else-if="detail">
      ...
      <div class="px-5 pt-4 pb-3 border-b border-pb-default">
        <div class="min-w-0">
          <h2 class="text-base font-bold text-pb-primary truncate font-mono">
            {{ detail.pod.name }}
          </h2>
          <div class="flex items-center gap-2 mt-1 flex-wrap">
            ...
            <span v-if="detail.pod.status_reason" class="text-[10px] text-pb-muted">
        ...
        <div class="mt-3 grid grid-cols-2 gap-x-6 gap-y-2 text-xs">
          <div>
            <p class="text-[10px] text-pb-muted font-bold uppercase tracking-widest mb-0.5">
              Node
            </p>
          ...
          <div>
            ...
            <p
              :class="detail.pod.restart_count > 0 ? 'text-pb-status-warn' : 'text-pb-secondary'"
          ...
          <div>
            <p class="text-[10px] text-pb-muted font-bold uppercase tracking-widest mb-0.5">Age</p>
      ...
      <div class="flex border-b border-pb-default px-5">
        ...
        >
          {{ tab.label }}
          <span v-if="tab.key === 'containers'" class="ml-1 text-[10px] text-pb-muted">{{
        ...
        <template v-if="activeTab === 'containers'">
          ...
          <div v-else class="space-y-2">
            ...
            >
              <div class="flex items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2">
                  <span
                    ...
                    class="text-[10px] font-bold uppercase tracking-wider text-pb-status-down bg-pb-status-down border border-pb-sev-incident px-1.5 py-0.5 rounded"
                    >not ready</span
                  >
                  <span
                    :class="[
              ...
              <div class="mt-1.5 text-xs text-pb-muted font-mono truncate">
                {{ container.image }}
              </div>
              <div class="flex items-center gap-4 mt-1 text-xs text-pb-muted">
                <span v-if="container.state_reason" class="text-pb-status-warn">{{
                  container.state_reason
                }}</span>
                <span v-if="container.restart_count > 0" class="text-pb-status-warn"
    ...
    <div v-else class="flex items-center justify-center py-16">
      <span class="text-sm text-pb-muted">Pod not found.</span>
    </div>
  </div>
</template>
