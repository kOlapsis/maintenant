...
function memColor(percent: number | null): string {
  if (percent === null) return 'bg-pb-elevated'
  if (percent > 85) return 'bg-pb-sev-incident-solid'
  if (percent > 60) return 'bg-pb-sev-warning-solid'
...
function podStatusStyle(status: string): string {
  ...
  if (s === 'failed') return 'text-pb-status-down bg-pb-status-down border-pb-sev-incident'
  return 'text-pb-muted bg-pb-elevated border-pb-default'
}
...
function conditionStatusStyle(status: string): string {
  ...
  if (status === 'False') return 'text-pb-status-down bg-pb-status-down border-pb-sev-incident'
  return 'text-pb-status-warn bg-pb-status-warn border-pb-sev-warning'
}

function eventTypeStyle(type: string): string {
  if (type === 'Warning') return 'text-pb-status-warn bg-pb-status-warn border-pb-sev-warning'
  return 'text-pb-secondary bg-pb-elevated border-pb-default'
}
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div v-if="loading" class="flex items-center justify-center py-16">
      <span class="text-sm text-pb-muted">Loading workload…</span>
    </div>

    <template v-else-if="detail">
      <!-- Header -->
      <div class="px-5 pt-4 pb-3 border-b border-pb-default">
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <h2 class="text-base font-bold text-pb-primary truncate">{{ detail.workload.name }}</h2>
            <div class="flex items-center gap-2 mt-1 flex-wrap">
              <span
                class="text-[10px] font-bold uppercase tracking-wider text-pb-secondary bg-sky-400/10 border border-sky-400/20 px-1.5 py-0.5 rounded"
              >
                {{ detail.workload.kind }}
              </span>
              <span
                class="text-[10px] font-mono text-pb-muted bg-pb-elevated border border-pb-default px-1.5 py-0.5 rounded"
              >
        ...
        <!-- Timestamps -->
        <div class="mt-3 flex gap-6 text-xs text-pb-muted">
          <span
            >Created
            <span class="text-pb-muted">{{ timeAgo(detail.workload.created_at) }}</span></span
          >
          <span
            >Updated
            <span class="text-pb-muted">{{ timeAgo(detail.workload.last_transition) }}</span></span
          >
        </div>
      </div>

      <!-- Tabs -->
      <div class="flex border-b border-pb-default px-5">
        <button
          v-for="tab in tabs"
          :key="tab.key"
          :class="[
            ...
            activeTab === tab.key
              ...
              : 'border-transparent text-pb-muted hover:text-pb-secondary',
          ]"
          @click="onTabClick(tab.key)"
        >
          {{ tab.label }}
          <span v-if="tab.key === 'pods'" class="ml-1 text-[10px] text-pb-muted">{{
            detail.pods.length
          }}</span>
          <span v-if="tab.key === 'events'" class="ml-1 text-[10px] text-pb-muted">{{
            detail.events.length
          }}</span>
          <span v-if="tab.key === 'conditions'" class="ml-1 text-[10px] text-pb-muted">{{
      ...
      <div class="flex-1 overflow-y-auto px-5 py-4">
        ...
        <template v-if="activeTab === 'pods'">
          <div v-if="detail.pods.length === 0" class="text-sm text-pb-muted py-4 text-center">
            No pods
          </div>
          <div v-else class="space-y-1">
            <div
              ...
              class="bg-pb-primary rounded-lg border border-pb-default px-4 py-3"
            >
              ...
              <div class="flex items-center gap-4 mt-1.5 text-xs text-pb-muted">
                <span v-if="pod.node_name" class="font-mono">{{ pod.node_name }}</span>
                <span v-if="pod.pod_ip" class="font-mono">{{ pod.pod_ip }}</span>
                <span v-if="pod.restart_count > 0" class="text-pb-status-warn"
        ...
        <template v-else-if="activeTab === 'events'">
          <div v-if="detail.events.length === 0" class="text-sm text-pb-muted py-4 text-center">
            No events
          </div>
          <div v-else class="space-y-1">
            <div
              ...
              <div class="flex items-start gap-3">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <span v-if="event.count > 1" class="text-[10px] text-pb-muted tabular-nums"
                      >×{{ event.count }}</span
                    >
                  </div>
                  <p class="text-xs text-pb-muted mt-0.5 leading-relaxed">{{ event.message }}</p>
                  <div class="flex items-center gap-3 mt-1 text-xs text-pb-muted">
        ...
        <template v-else-if="activeTab === 'conditions'">
          <div
            ...
            class="text-sm text-pb-muted py-4 text-center"
          >
            No conditions
          </div>
          <div v-else class="space-y-1">
            ...
            >
              ...
              <div v-if="condition.reason" class="mt-1 text-xs text-pb-muted font-semibold">
                {{ condition.reason }}
              </div>
              <p v-if="condition.message" class="mt-0.5 text-xs text-pb-muted leading-relaxed">
                {{ condition.message }}
              </p>
              <div class="mt-1 text-xs text-pb-muted">
        ...
        <template v-else-if="activeTab === 'resources'">
          ...
          >
            <div v-if="resourcesLoading" class="text-sm text-pb-muted py-4 text-center">
              Loading resources...
            </div>
            <div v-else-if="!metricsAvailable" class="text-center py-8">
              <p class="text-sm text-pb-muted font-medium mb-1">Metrics unavailable</p>
            ...
            <div v-else class="space-y-2">
              ...
              >
                <div class="flex items-center justify-between mb-2">
                  ...
                  <span v-if="pr.node_name" class="text-xs text-pb-muted font-mono">{{
                    pr.node_name
                  }}</span>
                </div>
                ...
                <div class="mb-2">
                  <div class="flex items-center justify-between mb-1">
                    <span class="text-[10px] text-pb-muted font-bold uppercase tracking-widest"
                      >CPU</span
                    >
                    <span class="text-xs text-pb-muted tabular-nums">{{
                ...
                <div>
                  <div class="flex items-center justify-between mb-1">
                    ...
                    <span class="text-xs text-pb-muted tabular-nums">
                      ...
                      >
                      <span v-if="pr.mem_percent !== null" class="text-pb-muted ml-1"
                  ...
                  <div
                    ...
                    class="h-1.5 bg-pb-primary border border-pb-default rounded-full overflow-hidden"
                  >
                    <div
    ...
    <div v-else class="flex items-center justify-center py-16">
      <span class="text-sm text-pb-muted">Workload not found.</span>
    </div>
  </div>
</template>
