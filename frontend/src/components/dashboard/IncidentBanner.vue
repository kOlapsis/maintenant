...
const severityDotClasses: Record<string, string> = {
  critical: 'bg-pb-sev-incident-solid shadow-[0_0_8px_rgba(244,63,94,0.5)]',
  warning: 'bg-pb-sev-warning-solid shadow-[0_0_8px_rgba(245,158,11,0.4)]',
...
<template>
  <div
    ...
    class="bg-pb-surface rounded-2xl border border-pb-default p-4 mb-4 shadow-lg"
  >
    <div class="flex items-center justify-between mb-3">
      ...
      <button
        ...
        class="p-1.5 rounded-lg text-pb-muted hover:text-pb-primary hover:bg-pb-elevated transition-colors"
        aria-label="Dismiss"
      >
        <X :size="14" />
      </button>
    ...
    <div
      ...
      :key="incident.id"
      class="border-t first:border-t-0 border-pb-default/50"
    >
      <!-- Main incident row -->
      <div
        class="flex items-center gap-3 py-2.5 cursor-pointer rounded-lg transition-colors hover:bg-pb-elevated border-l-2 pl-3"
      ...
      >
        ...
        <span
          ...
          :class="severityDotClasses[incident.severity] || 'bg-pb-sev-neutral-solid'"
        />
        ...
        <div class="flex-1 min-w-0">
          ...
          <span class="text-xs ml-2 text-pb-muted">
            {{ incident.message }}
          </span>
        </div>
        ...
        <div class="hidden sm:flex items-center gap-1">
          <template v-for="(step, idx) in timelineSteps" :key="step">
            <span
              ...
              :class="
                incident.states.some(s => s.status === step)
                  ...
                  : 'bg-transparent border-pb-default'
              "
            ...
            <span
              ...
              class="w-3 h-px"
              :class="
                incident.states.some(s => s.status === step) ? 'bg-pb-sev-neutral-solid' : 'bg-pb-elevated'
              "
            />
          </template>
        </div>
      ...
      >
        <div class="relative ml-2">
          ...
          <div class="absolute left-[5px] top-0 h-full w-px bg-pb-elevated" />
          ...
          >
            ...
            <div
              ...
              :class="
                incident.states.some(s => s.status === step)
                  ? severityDotClasses[incident.severity]
                  : 'bg-pb-surface border-pb-default'
              "
            />
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2">
                <span
                  ...
                  :class="incident.states.some(s => s.status === step) ? 'text-pb-primary' : 'text-pb-muted'"
                >
                  {{ step }}
                </span>
                <span
                  v-if="getStepTimestamp(incident, step)"
                  class="text-[11px] text-pb-muted"
                >
                  {{ formatFullTimestamp(getStepTimestamp(incident, step)!) }}
                </span>
                <span
                  v-else
                  class="text-[11px] italic text-pb-muted"
                >
                  pending
                </span>
              </div>
              <p
                ...
                class="text-[11px] mt-0.5 text-pb-muted"
              >
