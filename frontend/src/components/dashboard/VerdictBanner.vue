  ...
  >
    ...
    >
      <span
        ...
        class="pb-ping absolute inset-0 rounded-xl"
    ...
    <div class="min-w-0">
      <h1 class="text-base font-semibold tracking-tight text-pb-primary sm:text-lg">{{ title }}</h1>
      <p v-if="summary" class="mt-0.5 truncate text-xs text-pb-muted sm:text-sm">{{ summary }}</p>
    </div>
    ...
    <div class="ml-auto flex shrink-0 gap-2 sm:gap-2.5">
      <div
        v-for="c in counters"
        :key="c.label"
        class="flex min-w-[58px] flex-col items-center rounded-xl border border-pb-default bg-pb-surface px-3 py-2"
      >
        <span class="text-lg font-bold leading-none sm:text-xl" :style="{ color: c.color }">{{ c.value }}</span>
        <span class="mt-1 text-[10px] uppercase tracking-wide text-pb-muted">{{ c.label }}</span>
      </div>
    </div>
  </div>
</template>
